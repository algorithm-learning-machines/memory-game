...
--------------------------------------------------------------------------------
-- Player to test
cmd:option("--player", "S", "Who has to play? (Q/rand/batman/DeepQ)")
...
elseif opt.player == "S" then
   Player = require("SARSAPlayer")
end
   ...
   for e = 1, evalEvery do
      ...
      while not game:isOver() do
         ...
         if not game:isOver() then
            newAction = player:selectAction(state, actionsAvailable, true)
         else
            break
         end

         player:feedback(oldState, action, reward, state, newAction)
